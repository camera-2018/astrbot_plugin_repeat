...
| `recall_cooldown_seconds` | 同一群内同一句回复再次发送的冷却秒数(0=关闭) | 600 |
...
- **浏览 / 原文查询**:按群 / 模式分页翻看,并可**不走向量**直接按存储原文查询——
  关键词(对 text/response 精确子串包含,支持中文、大小写无关)、发送者 ID、入库时间范围;
  每条结果显示发送者与入库时间。
  > 关键词查询用的是自建 **bigram(双字符窗口)倒排索引**加速,不是 Qdrant 的原生全文索引——
  > 实测 Qdrant `MatchText` 对中文场景基本不可用(分词后按 token 匹配,查询"火锅"经常搜不到
  > 明明包含"我爱吃火锅"的记录),所以写入时按 2 字符窗口预计算索引 key 存进 Qdrant,查询时
  > 先用索引快速圈定候选、再做一次精确子串校验排除假阳性——结果精确,且比全量扫描快得多。
  > 插件升级前已存在的旧记忆点没有这个索引,会自动走兜底扫描继续被搜到,**不需要手动迁移**,
  > 随着新消息不断写入,这批"旧数据"会自然减少。
- **新增 / 编辑**:手动添加或按 id 覆盖记忆点(echo 填要翻出来的话;cont 填触发词 + 接续句),保存时自动重算向量。
- **删除 / 清空**:支持勾选多条批量删除、删除单条,或一键清空整群。
...
- **先检索后写入**,避免命中刚插入的自己。
- 收集与回复相互独立:发话受 `每群冷却 + 同一句冷却 + 概率` 控制,收集只看用户白名单。
- 同一句回复成功发出后,该群 10 分钟内不会再发相同内容;最高分候选在冷却时会尝试其他达标候选。附和与顺延输出相同内容也共用这段冷却。
- 复读命中时使用主动消息独立发送,不会占用原消息事件的处理结果；若原消息同时唤醒了
  AstrBot Agent，复读和 Agent 正常回复都会执行。
...
- **At 提及清洗**:存储/匹配前会清洗 @ 提及产生的噪声——优先从消息组件链只取 Plain 文本
  (丢弃 At/Reply 等组件);对某些平台会把 At 解析失败退化成 `@昵称(QQ号)` 字面文本混进正文的情况,
  再用窄口径正则清洗掉(只匹配"昵称后紧跟括号+5 位以上数字"的形状,不会误伤"@张三今晚吃啥"这类正常提及)。
