...
- **浏览 / 原文查询**:按群 / 模式分页翻看,并可**不走向量**直接按存储原文查询——
  ...
  每条结果显示发送者与入库时间。海量数据按页扫描,带上限并回报"已扫描 N 条"。
  > 关键词查询用的是客户端子串匹配,不是 Qdrant 的全文索引——实测 Qdrant `MatchText`
  > 对中文场景基本不可用(分词后按 token 匹配,查询"火锅"经常搜不到明明包含
  > "我爱吃火锅"的记录),所以这里换成结果精确、但量大时稍慢的子串扫描。
